...
async function api(path: string, body?: unknown) {
  ...
  if (cleanPath.startsWith('deception/') && sessionStorage.getItem('ns_deception_emulated') === 'true') {
...
export function Intelligence({ initialView = 'campaigns' }: { initialView?: 'campaigns' | 'deception' }) {
  ...
  const load = async () => {
    ...
    // If Docker daemon is unavailable, enable Emulated Sandbox Mode so Honeytrapping can be tested & demoed
    if (!state?.runtimeAvailable || sessionStorage.getItem('ns_deception_emulated') === 'true') {
      if (!i || i.length === 0) {
        i = mockIncidents;
      }
      state = {
        ...
        runtimeAvailable: true,
        isEmulated: true,
        enabled: Boolean(mockDeceptionStatus.enabled)
      };
      if (!s || s.length === 0) {
        s = mockDeceptionSessions;
      }
    }
