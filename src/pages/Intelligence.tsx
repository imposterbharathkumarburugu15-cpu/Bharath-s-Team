...
// Operational SOC state comes only from the authenticated backend.
async function api(path: string, body?: unknown) {
  const cleanPath = path.replace(/^\//, '');
  const response = await fetch(`/api/${cleanPath}`, {
    method: body === undefined ? 'GET' : 'POST',
    headers: { Authorization: `Bearer ${socKey}`, 'Content-Type': 'application/json', Accept: 'application/json' },
    body: body === undefined ? undefined : JSON.stringify(body),
    signal: AbortSignal.timeout(15000),
  });
  if (!response.headers.get('content-type')?.includes('application/json')) {
    throw new Error('SOC backend unavailable. No operational data or actions can be verified.');
  }
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || `SOC request failed (${response.status}).`);
  return data;
}
...
export function Intelligence({ initialView = 'campaigns' }: { initialView?: 'campaigns' | 'deception' }) {
  ...
  const load = async () => {
    let [c, i, t, s, state] = await Promise.all([
      api('intelligence/campaigns'),
      api('intelligence/incidents'),
      api('intelligence/iocs'),
      api('deception/sessions'),
  ...
  return <main className="ns-workspace"><div className="ns-eyebrow">{t('soc_eyebrow')}</div><div className="ns-section-heading"><h1>{t('soc_title')}<span className="ns-accent">.</span></h1>{role && <button className="ns-secondary" onClick={() => { persistKey(''); setRole(''); setIssued(null); setClusters([]); setIncidents([]); setSessions([]); setIOCs([]); }}>{t('soc_lock_btn')}</button>}</div><p className="ns-lead">{t('soc_desc')}</p>
    {!role ? (
      ...
      >
        ...
        <p className="ns-caption">Use the individual SOC key configured by your administrator. To explore without credentials, open Evidence Lab for explicitly synthetic test cases.</p>
